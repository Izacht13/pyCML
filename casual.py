...
def __tagfunc_riter__(element, args):
	if not (element.parent and args): return
	for child in element:
		l = len(child.content)
		attributes = [Attribute(args[0], child.tag)]
		for i, tag in enumerate(args[1:]):
			attributes.append(Attribute(tag, child.content[i] if i < l and child.content != ' ' else None))
		element.parent.children.append(Element(element.tag, attributes=attributes))
...
class Element(TaggedContent):
	...
	def child(self, tag, content=None, attributes=None, children=None, *, extend=False):
		...
		try:
			...
			if content:
				if extend: child.content.extend(content)
				else: child.content = content
			if attributes: child.attribute = attributes
	...
	def attribute(self, tag, content=None, *, extend=False):
		for attribute in self.attributes:
			if attribute == tag:
				if content:
					if extend: attribute.content.extend(content)
					else: attribute.content = content
				return attribute
